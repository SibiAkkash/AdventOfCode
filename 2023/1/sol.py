...
def first_and_last_digit(input: str) -> int:
    ...
    while (
        (first_num == -1 or last_num == -1)
        and (left <= right)
        and (left < len(input) and right >= 0)
    ):
        if first_num == -1:
...
def first_and_last_digit_words_re(
    first_pattern: re.Pattern, last_pattern: re.Pattern, input: str
) -> int:
    # we're guarenteed to have atleast 1 number/word in the string
    # so ignoring the type errors
...
if __name__ == "__main__":
    ...
    with open("./input.txt", "r") as file:
        total = sum(
            first_and_last_digit_words_re(first_pattern, last_pattern, line.strip())
